...
public class BLocationManagerService extends IBLocationManagerService.Stub implements ISystemService {
    ...
    public void setPattern(int userId, String pkg, int pattern) {
        synchronized (mLocationConfigs) {
            KailLog.i(null, "[sandbox]BLocationManagerService", "setPattern(userId=" + userId + " pkg=\"" + pkg + "\" pattern=" + pattern + ")");
            getOrCreateConfig(userId, pkg).pattern = pattern;
            String modeStr = pattern == BLocationManager.GLOBAL_MODE ? "GLOBAL" : (pattern == BLocationManager.OWN_MODE ? "OWN" : "CLOSE");
            KailLog.i(null, "[sandbox]BLocationManagerService", "setPattern: stored pkg=\"" + pkg + "\" pattern=" + pattern + " (" + modeStr + ")");
            save();
        }
    }
    ...
    public BLocation getLocation(int userId, String pkg) {
        BLocationConfig config = resolveConfig(userId, pkg);
        KailLog.i(null, "[sandbox]BLocationManagerService", "getLocation(userId=" + userId + " pkg=\"" + pkg + "\") resolved pattern=" + config.pattern);
        switch (config.pattern) {
            case BLocationManager.OWN_MODE:
                KailLog.i(null, "[sandbox]BLocationManagerService", "getLocation: OWN_MODE -> config.location=" + (config.location == null ? "null" : "(" + config.location.getLatitude() + "," + config.location.getLongitude() + ")"));
                return config.location;
            case BLocationManager.GLOBAL_MODE: {
                BLocation loc = mGlobalConfig.location;
                KailLog.i(null, "[sandbox]BLocationManagerService", "getLocation: GLOBAL_MODE -> mGlobalConfig.location=" + (loc == null ? "null" : "(" + loc.getLatitude() + "," + loc.getLongitude() + ")"));
                return loc;
            }
            case BLocationManager.CLOSE_MODE:
            default:
                KailLog.i(null, "[sandbox]BLocationManagerService", "getLocation: CLOSE_MODE -> returning null (real location will be used)");
                return null;
        }
    }
    ...
    public void setGlobalLocation(BLocation location) {
        synchronized (mGlobalConfig) {
            KailLog.i(null, "[sandbox]BLocationManagerService", "setGlobalLocation: lat=" + location.getLatitude() + " lng=" + location.getLongitude());
            mGlobalConfig.location = location;
            save();
            KailLog.i(null, "[sandbox]BLocationManagerService", "setGlobalLocation: stored OK");
        }
    }
    ...
    public BLocation getGlobalLocation() {
        synchronized (mGlobalConfig) {
            BLocation loc = mGlobalConfig.location;
            KailLog.i(null, "[sandbox]BLocationManagerService", "getGlobalLocation: returning " + (loc == null ? "null" : "(" + loc.getLatitude() + "," + loc.getLongitude() + ")"));
            return loc;
        }
    }
    ...
    private void addTask(IBinder locationListener) {
        mThreadPool.execute(() -> {
            ...
            while (locationListener.pingBinder()) {
                ...
                pushCount++;
                KailLog.i(null, "[sandbox]BLocationManagerService", "addTask: pushing location #" + pushCount + " to pkg=" + locationRecord.packageName + " loc=(" + location.getLatitude() + "," + location.getLongitude() + ")");
