...
public class ILocationManagerProxy extends BinderInvocationStub {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        KailLog.i(null, "[sandbox]ILocationManagerProxy", "invoke: " + method.getName() + " called by pkg=" + pkg + " fakeEnabled=" + BLocationManager.isFakeLocationEnable());
    ...
    public static class GetLastKnownLocation extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            if (BLocationManager.isFakeLocationEnable()) {
                BLocation bl = BLocationManager.get().getLocation(BActivityThread.getUserId(), BActivityThread.getAppPackageName());
                if (bl == null) {
                    KailLog.w(null, "[sandbox]ILocationManagerProxy", "getLastKnownLocation: BLocation is null, passing to real system");
                    try {
                        return method.invoke(who, args);
                    } catch (Exception e) {
                        return null;
                ...
                android.location.Location loc = bl.convert2SystemLocation();
                KailLog.i(null, "[sandbox]ILocationManagerProxy", "getLastKnownLocation: returning fake loc=(" + loc.getLatitude() + "," + loc.getLongitude() + ")");
                return loc;
            }
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            if (BLocationManager.isFakeLocationEnable()) {
                if (args[1] instanceof IInterface) {
                    ...
                    String pkg = BActivityThread.getAppPackageName();
                    KailLog.i(null, "[sandbox]ILocationManagerProxy", "requestLocationUpdates: registering listener for pkg=" + pkg + " uid=" + BActivityThread.getUserId());
                    BLocationManager.get().requestLocationUpdates(listener.asBinder());
                    return 0;
                }
                KailLog.w(null, "[sandbox]ILocationManagerProxy", "requestLocationUpdates: args[1] is not IInterface, type=" + (args[1] == null ? "null" : args[1].getClass().getName()));
            }
