...
import com.kail.location.root.RootModeClient
...
class ServiceGoRoot : Service() {
    ...
    private val mRootModeClient = RootModeClient(this)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            if (!ctrl.isNullOrBlank()) {
                when (ctrl) {
                    ...
                    CONTROL_SET_SPEED -> {
                        ...
                        mRootModeClient.currentSpeed = mSpeed
                        mRootModeClient.start()
                        mRootModeClient.writeConfig()
                        return super.onStartCommand(intent, flags, startId)
                    }
                    ...
                    CONTROL_SET_STEP -> {
                        val stepEnabled = intent.getBooleanExtra(EXTRA_STEP_ENABLED, mRootModeClient.stepEnabled)
                        val stepFreq = intent.getFloatExtra(EXTRA_STEP_FREQ, mRootModeClient.stepFreq.toFloat()).toDouble()
                        mRootModeClient.stepEnabled = stepEnabled
                        mRootModeClient.stepFreq = stepFreq
                        mRootModeClient.start()
                        mRootModeClient.writeConfig()
                        if (stepEnabled && stepFreq > 0) {
                            NativeSensorHook.setRouteSimulation(true, stepFreq.toFloat(), 0)
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                }
            }
            mRootModeClient.stepEnabled = intent.getBooleanExtra(EXTRA_STEP_ENABLED, false)
            mRootModeClient.stepFreq = intent.getFloatExtra(EXTRA_STEP_FREQ, 0f).toDouble()
            speedFluctuation = intent.getBooleanExtra(EXTRA_SPEED_FLUCTUATION, false)
            mRootModeClient.simScheme = PreferenceManager.getDefaultSharedPreferences(this).getString("setting_sim_scheme", "0")?.toIntOrNull() ?: 0
            mRootModeClient.stepSimEnabled = PreferenceManager.getDefaultSharedPreferences(this).getBoolean("setting_step_sim_enabled", true)
        }
        ...
        if (intent != null) {
            ...
            val cellOnly = intent.getBooleanExtra(EXTRA_CELL_ONLY, false)
            val isLightMode = wifiOnly || cellOnly

            if (isLightMode) {
                KailLog.i(this, "ServiceGoRoot", "onStartCommand light mode: wifiOnly=$wifiOnly cellOnly=$cellOnly")
                if (this::mLocHandler.isInitialized) {
                    mLocHandler.post { mRootModeClient.init() }
                }
            } else {
                ...
                mRootModeClient.currentLat = mCurLat
                mRootModeClient.currentLng = mCurLng
                mRootModeClient.currentAlt = mCurAlt
                mRootModeClient.currentBea = mCurBea
                mRootModeClient.currentSpeed = mSpeed
                ...
                if (this::mLocHandler.isInitialized) {
                    mLocHandler.post {
                        mRootModeClient.init()
                        mRootModeClient.start()
                        mRootModeClient.updateLocation()
                    }
                }
                ...
                if (stepEnabled) {
                    NativeSensorHook.setRouteSimulation(true, stepFreq, 0)
                }
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            mRootModeClient.stopAll()
            mRootModeClient.stepEnabled = false
            mRootModeClient.stepFreq = 0.0
            mRootModeClient.writeConfig()
            NativeSensorHook.setRouteSimulation(false, 0f, 0)
